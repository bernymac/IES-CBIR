...
import static NovaSYS.IES_CBIR.test.SSE_OPE.nBlockBySide;
...
public class ImgCryptSSE_OPE {
	...
	public ImgCryptSSE_OPE() throws Exception {
...
//	}
	...
	public ImgCiphertext encrypt (ImgCiphertext img) {
		...
		try {
			...
			int[] randoms = prng.generateRandomsVector(imgKey, (2*nBlockBySide)+blockWidth+blockHeight);
			...
			for (int h=0; h<height; h+=blockHeight) {					//shuffle all blocks by columns
				final int rnd = Math.abs(randoms[nBlockBySide+h/blockHeight])%width;
			...
			for (int w=0; w<width; w+=blockWidth) {					//shuffle rows and cols inside each block
				for (int h=0; h<height; h+=blockHeight) {
					...
					for (int i = w; i < widthLimit; i++) {				//shuffle block cols
						final int rndCols = Math.abs(randoms[2*nBlockBySide+(i-w)])%(heightLimit-h);
					...
					for (int j = h; j < heightLimit; j++) { 			//shuffle block rows
						final int rndRows = Math.abs(randoms[2*nBlockBySide+blockWidth+(j-h)])%(widthLimit-w);
